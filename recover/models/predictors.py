...
import numpy as np
from recover.models.utils import get_batch, get_layer_dims
...
class AbstractPredictor(torch.nn.Module):
    ...
    def forward(self, data, drug_drug_batch, h_drug, n_forward_passes=1):
        ...
        out = torch.empty([drug_drug_batch[0].shape[0], 0])
...
########################################################################################################################
# Concentration Only MLP Predictor
########################################################################################################################
...
class FilmMLPPredictor(MLPAbstractPredictor):
    ...
    def __init__(self, data, config):
        ...
        for i in range(len(self.layer_dims) - 1):
            layers.append(nn.Linear(self.layer_dims[i], self.layer_dims[i + 1]))
            film.append(Parameter(1/100 * torch.randn(self.num_cell_lines, 2) + torch.Tensor([[1, 0]])))
    ...
    def single_forward_pass(self, batch, cell_lines):
        ...
        for i in range(len(self.layers)):
            ...
            if i != len(self.layers) - 1:
                # Film conditioning
                x = self.film[i][cell_lines][:, :1] * x + self.film[i][cell_lines][:, 1:]
                x = F.relu(x)
