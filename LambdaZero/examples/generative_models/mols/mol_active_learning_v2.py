...
class Docker:
    def __init__(self, tmp_dir, cpu_req=2):
        ...
        # todo [maksym] something is not alright with docking setup; updated LambdaZero.chem
...
class Proxy:
    ...
    def train(self, dataset):
        print("proxy dataset", [m.reward for m in dataset.train_mols])

        self.reset()
        stop_event = threading.Event()
        best_model = self.proxy
        best_test_loss = 1000
        opt = torch.optim.Adam(self.proxy.parameters(), self.args.proxy_learning_rate,
                               betas=(self.args.proxy_opt_beta, 0.999),
        ...
        for i in range(self.args.proxy_num_iterations + 1):
            ...
            #print("mol out s", mol_out_s.detach().cpu().numpy(), "r", r.detach().cpu().numpy())
            loss.backward()
...
def main(args):
    ...
    proxy_dataset.load_h5(osp.join(datasets_dir, "brutal_dock/seh", "dock_db_1619111711tp_2021_04_22_13h.h5"),
