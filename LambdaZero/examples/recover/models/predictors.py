...
class FilmMLPPredictor(MLPAbstractPredictor):
    ...
    def __init__(self, data, config):
        ...
        for i in range(len(self.layer_dims) - 1):
            ...
            film_init = 1/100 * torch.randn(self.num_cell_lines, 2 * out_dim)
            film_init = film_init + torch.Tensor([([1] * out_dim) + ([0] * out_dim)])

            film.append(Parameter(film_init))
    ...
    def single_forward_pass(self, batch, cell_lines):
        ...
        for i in range(len(self.layers)):
            ...
            if i != len(self.layers) - 1:
                # Film conditioning
                x = self.film[i][cell_lines][:, :x.shape[1]] * x + self.film[i][cell_lines][:, x.shape[1]:]
                x = F.relu(x)
