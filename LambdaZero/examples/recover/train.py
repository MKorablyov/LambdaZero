import torch
from recover.datasets.drugcomb_score_l1000_data import DrugCombScoreL1000NoPPI
from recover.models.models import GiantGraphGCN
from recover.models.message_conv_layers import FourMessageConvLayer
import os
# from hyperopt import hp
from recover.models.predictors import SharedLayersMLPPredictor
from recover.utils import get_project_root
from torch.utils.data import TensorDataset, DataLoader
from recover.models.acquisition import RandomAcquisition, ExpectedImprovement, GreedyAcquisition
...
class AbstractTrainer(tune.Trainable):
    def _setup(self, config):
        ...
        self.train_idxs, self.val_idxs, self.test_idxs = dataset.random_split(config["test_set_prop"],
                                                                              config["val_set_prop"])

        # Valid loader
        valid_ddi_dataset = TensorDataset(self.data.ddi_edge_idx[:, self.val_idxs].T,
...
class BasicTrainer(AbstractTrainer):
    def _setup(self, config):
        ...
        # Train loader
        train_ddi_dataset = TensorDataset(self.data.ddi_edge_idx[:, self.train_idxs].T,
    ...
    def _train(self):
        train_metrics = self.train_epoch(self.data, self.train_loader, self.model, self.optim,
...
class ActiveTrainer(AbstractTrainer):
    ...
    def _train(self):
        ...
        seen_metrics = self.train_epoch(self.data, self.seen_loader, self.model, self.optim, self.max_examp_per_epoch,
                                        n_forward_passes=self.n_train_forward_passes)
...
if __name__ == '__main__':
    ...
    predictor_config = {
        "predictor": SharedLayersMLPPredictor,
    ...
    active_learning_config = {
        "acquisition": ExpectedImprovement,  # tune.grid_search([ExpectedImprovement, RandomAcquisition, GreedyAcquisition]),
        "kappa": 0.5,
        "dropout_proba": 0.4,
