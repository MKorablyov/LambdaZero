...
class SimDockLet: # Docking Class that lets us evaluate a molecule...
...
class MolMDPNC(MolMDP): # Molecule MDP No Cache
...
class GraphAgent(nn.Module): # Agent that takes as input a graph -- and outputs stem predictions and molecule predictions
...
class MolEpisode: # An episode of generating a moelcule
...
class ReplayBuffer: # Replay buffer of the episodes
...
class RolloutActor(threading.Thread): # Policy
    def __init__(self, bpath, device, model, stop_event, replay, synth_net, sample_type):
        ...
        self.beam_max = 6 # Beam search -- best first search?
...
def main(args):
    ...
    for i in range(args.num_iterations):
        ...
        negloglike = model_negloglikelihood(s, a, g, stem_o, mol_o).mean() # Ah this isn't behaviour cloning? -- No it's not
        logit_reg = (mol_o.pow(2).sum() + stem_o.pow(2).sum()) / (np.prod(mol_o.shape) + np.prod(stem_o.shape)) # Regularisation
