...
if __name__ == "__main__":
    ...
    single_block_blocks = (
        []
    )  # convenient data structure to have in hand when running this script interactively.
    ...
    normalized_value_count_series = df["attachment_block_class"].value_counts(
        normalize=True
    )
    ...
    cumulative_counts_series.reset_index(drop=True).plot.line(
        drawstyle="steps", color="orange", lw=4, ax=ax2
    )
    ...
    ax1 = fig.add_subplot(111)
    df[["total_energy", "parent_energy", "binding_energy"]].plot.hist(
        bins=100, alpha=0.5, ax=ax1
    )
    ...
    block_indices, counts = np.unique(
        df["attachment_block_class"].values, return_counts=True
    )
    sorting_indices = np.argsort(counts)[::-1]
    block_indices = block_indices[sorting_indices]
    counts = counts[sorting_indices]

    list_common_block_smiles = [
        blocks_dict["block_smi"][f"{block_indices[i]}"] for i in range(10)
    ]
    list_common_blocks = [
        Chem.MolFromSmiles(smiles) for smiles in list_common_block_smiles
    ]
    ...
    img = Draw.MolsToGridImage(
        ...
        subImgSize=(200, 200),
        legends=[
            f"block {index}, count = {count}"
            for index, count in zip(block_indices, counts)
        ],
    )
    ...
    list_mols = [
        ...
        for smiles in list_smiles[:number_of_sample_molecules]
    ]
