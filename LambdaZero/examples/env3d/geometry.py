...
def get_center_of_mass(list_masses: np.array, list_positions: np.array) -> np.array:
    ...
    assert (
            len(list_masses.shape) == 1
    ), "error: list_masses should be a 1D array."

    assert (
            len(list_positions.shape) == 2
    ), "error: list_positions should be a 2D array."
...
def get_inertia_contribution(mass: float, relative_position: np.array) -> np.array:
    ...
    inertia_contribution = mass*np.dot(relative_position,relative_position)*np.eye(3)
