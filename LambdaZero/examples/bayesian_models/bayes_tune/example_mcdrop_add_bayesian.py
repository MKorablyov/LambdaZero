...
transform = T.Compose([LambdaZero.utils.Complete(),MakeFP()])
...
def train_epoch(loader, model, optimizer, device, config):
    ...
    for bidx,data in enumerate(loader):
        ...
        optimizer.zero_grad()
        #inp = torch.from_numpy(np.array(data['fp'])).to(device)
    ...
    # print('train ll: {}'.format(np.mean(_log_lik(epoch_targets_norm, epoch_logits, config,N))))
    print('train mse: {}'.format(scores['mse']))
    return scores
...
def eval_epoch(loader, model, device, config):
    ...
    for bidx, data in enumerate(loader):
       ...
       # inp = torch.from_numpy(np.array(data['fp'])).to(device)
        ...
        logits = model(data, do_dropout=False, drop_p=config["drop_p"])
        targets_norm = config["normalizer"].forward_transform(targets)
    ...
    scores = _compute_metrics(epoch_targets_norm, epoch_logits, config["normalizer"])
   # global N
   # print('eval ll: {}'.format(np.mean(_log_lik(epoch_targets_norm, epoch_logits, config,N))))
    print('eval mse: {}'.format(scores['mse']))
    return scores
...
class MCDropRegressor(BasicRegressor):
    ...
    def fit(self, train_loader=None, val_loader=None):
        ...
        fps_train = np.concatenate([d.fp for d in self.train_loader],axis=0)
        fps_val = np.concatenate([d.fp for d in self.val_loader],axis=0)
        ...
        for i, j in enumerate(self.train_loader):
            ...
            emb_train.append(newmodel(j.to(self.device),do_dropout=False, drop_p=self.config["drop_p"]).detach().cpu().numpy())
        ...
        for i, j in enumerate(self.val_loader):
            ...
            emb_val.append(newmodel(j.to(self.device),do_dropout=False, drop_p=self.config["drop_p"]).detach().cpu().numpy())
...
#------ for benchmarking ------ 
        # compute trianing embedding data variance
        tar_mean = np.mean(emb_train_tar)
        tar_std = np.std(emb_train_tar)

        print('benchmark mse: {}'.format(((emb_val_tar - tar_mean)**2).mean()))

        # n = len(self.val_loader)
        ll_benchmark = -0.5*np.mean(np.log(2*np.pi*tar_std**2) + ((emb_val_tar-tar_mean)**2/tar_std**2))
        print('benchmark ll: {}'.format(ll_benchmark))

#------ Train Bayesian linear with MPNN ------ 
        ...
        ll_Bayesian_MPNN = -0.5*np.mean(np.log(2*np.pi*predicted_std**2)  + ((emb_val_tar-predicted_mn)**2/predicted_std**2))
        #print('Bayesian linear with MPNN: {}'.format(ll_Bayesian_MPNN))


#------ train Bayesian linear on fingerprint without MPNN ------ 
        clf = linear_model.BayesianRidge(compute_score=True, fit_intercept=False)
        clf.fit(fps_train, emb_train_tar) 

        train_preds = clf.predict(fps_train)
        val_preds = clf.predict(fps_val)

        print('beval mse: {}'.format(((emb_val_tar - val_preds)**2).mean()))
        print('train mse: {}'.format(((emb_train_tar - train_preds)**2).mean()))
        
        pred = clf.predict(fps_val)
        scores = _compute_metrics(emb_val_tar, pred, self.config["normalizer"])

        # Get ll
        predicted_mn, predicted_std = clf.predict(fps_val,return_std=True)
        ll_Bayesian_fingerprint_only = -0.5*np.mean(np.log(2*np.pi*predicted_std**2)  + ((emb_val_tar-predicted_mn)**2/predicted_std**2))
        print('Bayesian on fingerprint: {}'.format(ll_Bayesian_fingerprint_only))

 
    def _get_sample(self, loader, model, drop=True):
        y_hat = []
        for bidx, data in enumerate(loader):
           # inp = torch.from_numpy(np.array(data['fp'])).to(self.device)
...
class MPNNetDrop2(LambdaZero.models.MPNNetDrop):
    ...
    def forward(self, data, do_dropout, drop_p):
        ...
        # out = nn.functional.dropout(nn.functional.relu(self.lin2(out)), training = do_dropout, p = drop_p)
