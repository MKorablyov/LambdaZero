...
class LinearDec(torch.nn.Module):
    def __init__(self, Z, R, D):
        ...
        self.D = torch.nn.Parameter(
            torch.empty(32, 32).normal_(mean=0, std=0.01), requires_grad=True
        )
...
def eval_epoch(loader, model, device, config):
    ...
    confusion = sklearn.metrics.confusion_matrix(
        labels_all, np.asarray((preds_all - preds_all.mean()) > 0, dtype=np.int32)
    )
...
class BasicRegressor(tune.Trainable):
    def _setup(self, config):
        ...
        self.train_loader = torch.utils.data.DataLoader(
            self.train_set, batch_size=config["b_size"], shuffle=True
        )
        self.test_set = Subset(dataset, np.where(~is_train)[0])
        self.test_loader = torch.utils.data.DataLoader(
            self.test_set, batch_size=config["b_size"]
        )

        # acquire
        self.is_aquired = np.zeros(len(self.train_set), dtype=np.bool)
        self.is_aquired[
            np.random.choice(np.arange(len(self.train_set)), config["aq_batch_size"])
        ] = True

        # make model
        self.model = LinearDec(
    ...
    def _train(self):
        ...
        aq_loader = torch.utils.data.DataLoader(
            aq_set, batch_size=self.config["b_size"]
        )
        # train model
        for i in range(3):
            train_scores = self.train_epoch(
                aq_loader, self.model, self.optim, self.device, self.config
            )
        eval_scores = self.eval_epoch(
            self.test_loader, self.model, self.device, self.config
        )
