...
class MolAC_GCN(GFlowModelBase):
    def __init__(self, cfg: Namespace, **kwargs):
        super(MolAC_GCN, self).__init__(cfg, **kwargs)

        nhid = getattr(cfg, "nemb", 256)
        nvec = getattr(cfg, "nvec", 0)
        num_out_per_stem = getattr(cfg, "out_per_stem", 105)
        num_out_per_mol = getattr(cfg, "out_per_mol", 1)
        num_conv_steps = getattr(cfg, "num_conv_steps", 10)
        version = getattr(cfg, "version", "v4")
        dropout_rate = getattr(cfg, "dropout_rate", 0)
        do_stem_mask = getattr(cfg, "do_stem_mask", True)
        do_nblocks = getattr(cfg, "do_nblocks", False)
    ...
    def forward(self, graph, vec=None, do_stems=True, do_bonds=False, k=None, do_dropout=False,
                **kwargs):
        per_stem_out, per_mol_out = self.mpnn(graph, vec, do_stems=do_stems, do_bonds=do_bonds, k=k, do_dropout=do_dropout, **kwargs)
