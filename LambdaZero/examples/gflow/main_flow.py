...
class Dataset:
    ...
    def _add_mol_to_online(self, r, m, inflow):
        if self.replay_mode == 'online':
            ...
            if len(self.online_mols) > self.max_online_mols:
                self.online_mols = sorted(self.online_mols, key=lambda x: x[0])[max(int(0.05 * self.max_online_mols), 1):]  # TODO this seems inefficient
    ...
    def _get_reward(self, m):
        ...
        return self.r2r(dockscore=self.proxy_reward(m))
    ...
    def sample(self, n):
        if self.replay_mode == 'dataset':
            ...
            print("GET count", len(eidx))
            samples = sum((self._get(i, self.online_mols) for i in eidx), [])
            print("RESULT GET count", len(samples))
