...
def mol2graph(mol, mdp, floatX=torch.float, bonds=False, nblocks=False):
    ...
    if bonds:
        ...
        # Construct bonds
        for ijbond in mol.jbonds:
            bonds_stems += [[ijbond[0], ijbond[2]], [ijbond[1], ijbond[3]]]

        bondstypes = [mdp.stem_type_offset[t[mol.blockidxs[i[0]]]] + i[1] for i in bonds_stems]

        data.bonds = f(bonds_stems) if len(bonds_stems) else f([]).view(0, 2)
        data.bondstypes = f(bondstypes) if len(bonds_stems) else f([])
...
def mols2batch(mols, mdp):
    follow_batch = ['stems']
    if hasattr(mols[0], "bonds"):
        follow_batch.append("bonds")

    batch = Batch.from_data_list(
        mols, follow_batch=follow_batch)
    batch.to(mdp.device)
    return batch
