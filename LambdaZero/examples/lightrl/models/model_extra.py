...
def one_hot(idx, device, max_cnt):
    hot = torch.zeros(len(idx), max_cnt, device=device).scatter_(1, idx.unsqueeze(1), 1.)
    return hot


class MPNNextra(ModelBase):
    ...
    def forward(self, inputs, rnn_hxs, masks, vec_data=None):
        ...
        out = torch.cat([out, rsteps], dim=1)
