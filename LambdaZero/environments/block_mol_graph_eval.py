...
class EvalCallback(DefaultCallbacks):
    _prev_training_iteration = 0
    _prev_timesteps_total = 0

    def on_episode_end(self, worker: "RolloutWorker",
                       base_env: BaseEnv,
                       policies: TypeDict[PolicyID, Policy],
    ...
    def on_postprocess_trajectory(
            ...
            policies: TypeDict[PolicyID, Policy], postprocessed_batch: SampleBatch,
            original_batches: TypeDict[AgentID, SampleBatch], **kwargs):

        if worker.env._eval_mode:
            worker.env.add_eval_data(original_batches[agent_id][1], postprocessed_batch)

    def on_train_result(self, trainer, result: dict, **kwargs) -> None:
        # TODO SO so ugly
        if getattr(trainer, "evaluation_workers", None) is not None:
            trainer.evaluation_workers._local_worker.env._prev_training_iteration = result['training_iteration']
            trainer.evaluation_workers._local_worker.env._prev_timesteps_total = result["timesteps_total"]
...
class BlockMolEnvGraph_Eval(BlockMolEnvGraph_v1):
    def __init__(self, config=None, *args, **kwargs):
        ...
        # Save config info
        config = self._config
        self._logger = config["reward_config"]["scoreProxy_config"]["logger"]
        ...
        self._log_dir = ray.get(config["logger"].log_dir.remote())
...
def env_creator(env_config):
    return BlockMolEnvGraph_Eval(env_config)  # return an env instance


register_env("BlockMolEnvGraph_Eval", env_creator)
