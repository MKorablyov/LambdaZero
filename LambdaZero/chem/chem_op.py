...
from LambdaZero.chem.chimera_op import (
    _add_hydrogens_and_compute_gasteiger_charges_with_chimera,
)
...
def break_on_bonds(mol, jun_bonds, frags_generic):
    ...
    for i in range(n_junct + 1):
        ...
        frag_coord.append(
            np.asarray(
                [frags[i].GetConformer(0).GetAtomPosition(j) for j in range(frag_natm)]
            )
        )
...
def mol_from_frag(jun_bonds, frags=None, frag_smis=None, coord=None, optimize=False):
    ...
    if frags is not None:
        ...
        raise ValueError(
            "invalid argument either frags or frags smis should be not None"
        )
    if len(frags) == 0:
        return None, None
    nfrags = len(frags)
    ...
    if optimize:
        assert not "h" in set(
            [atm.GetSymbol().lower() for atm in mol.GetAtoms()]
...
def build_mol(smiles=None, num_conf=1, minimize=False, noh=True, charges=True):
    ...
    coord = [
        np.asarray([mol.GetConformer(j).GetAtomPosition(i) for i in range(len(elem))])
...
def _gen_mol2(smi, mol_name, outpath, chimera_bin, charge_method, num_conf):
    ...
    _add_hydrogens_and_compute_gasteiger_charges_with_chimera(
        mol_file, charge_method, chimera_bin, mol2_file
    )
...
class Dock_smi:
    def __init__(
        ...
        if not trustme:
            assert os.path.exists(self.chimera_bin), (
                "can't find chimera bin " + self.chimera_bin
            )
        # dock6 bin and parameter files
        self.dock6_bin = os.path.join(dock6_dir, "bin/dock6")
        if not trustme:
            assert os.path.exists(self.dock6_bin), (
                "can't find dock6 bin " + self.dock6_bin
            )
        self.vdw_defn_file = os.path.join(dock6_dir, vdw_defn_file)
        if not trustme:
            assert os.path.exists(self.vdw_defn_file), (
                "can't find vdw_def_file " + self.vdw_defn_file
            )
        self.flex_defn_file = os.path.join(dock6_dir, flex_defn_file)
        if not trustme:
            assert os.path.exists(self.flex_defn_file), (
                "can't find flex_dfn_file " + self.flex_defn_file
            )
        self.flex_drive_file = os.path.join(dock6_dir, flex_drive_file)
        if not trustme:
            assert os.path.exists(self.flex_drive_file), (
                "can't find dock6 bin " + self.flex_drive_file
            )
        self.cleanup = cleanup
        # docking files
        self.rec_site_file = os.path.join(docksetup_dir, rec_site_file)
        if not trustme:
            assert os.path.exists(self.rec_site_file), (
                "can't find rec site file " + self.rec_site_file
            )
        self.grid_prefix = os.path.join(docksetup_dir, grid_prefix)
        self.dock_in_template = os.path.join(docksetup_dir, dock_in_template)
        if not trustme:
            assert os.path.exists(self.dock_in_template), (
                "can't find rec site file " + self.dock_in_template
            )
    ...
    def dock(self, smi, mol_name=None, molgen_conf=20):
        try:
            ...
            mol2_file = _gen_mol2(
                ...
                self.chimera_bin,
                self.charge_method,
                num_conf=molgen_conf,
            )
            gridscore, coord = self._dock_dock6(
                smi,
                mol2_file,
                mol_name,
                self.outpath,
                self.dock_in_template,
                self.dock6_bin,
            )
        finally:
            if self.cleanup:
    ...
    def _dock_dock6(self, smi, mol2file, mol_name, outpath, dock_pars_file, dock_bin):
        # write dockfile
        with open(dock_pars_file, "r") as textfile:
            dock_pars_ = textfile.read()
        dock_pars = dock_pars_.format(
            mol2file,
            self.rec_site_file,
            self.grid_prefix,
            self.vdw_defn_file,
            self.flex_defn_file,
            self.flex_drive_file,
        ...
        mol = Chem.MolFromMol2File(
            mol2file_scored, sanitize=False, cleanupSubstructures=False
        )
        atoms = np.asarray(
            [
                atm.GetSymbol().lower()
                for atm in mol.GetAtoms()
                if atm.GetSymbol().lower() != "h"
            ]
        )
        ...
        assert np.array_equal(
            atoms, initatoms
        ), "atoms are not the same after parsing dock mol2"

        noh = np.asarray([atm.GetSymbol().lower() != "h" for atm in mol.GetAtoms()])
        noh = np.where(noh)[0]
        coord = np.asarray(
            [mol.GetConformer(0).GetAtomPosition(int(idx)) for idx in noh]
        )
    ...
    def __init__(self, mols_smiles, incude_chirality=False):
        ...
        for i in range(len(mols_smiles)):
            ...
            scaffold = MurckoScaffold.MurckoScaffoldSmiles(
                mol=mol, includeChirality=incude_chirality
            )
    ...
    if from_atoms is not None:
        ...
        fps = [
            AllChem.GetMorganFingerprintAsBitVect(
                mol, r, fp_length, fromAtoms=from_atoms
            )
            for r in fp_radiis
        ]
        ...
        fps = [
            AllChem.GetMorganFingerprintAsBitVect(mol, r, fp_length) for r in fp_radiis
        ]
...
class FPEmbedding_v2:
    ...
    def __call__(self, molecule):
        ...
        stem_fps = [
            get_fp(mol, self.stem_fp_len, self.stem_fp_radiis, [idx])
            for idx in molecule.stem_atmidxs
        ]

        jbond_fps = [
            (
                get_fp(mol, self.stem_fp_len, self.stem_fp_radiis, [idx[0]])
                + get_fp(mol, self.stem_fp_len, self.stem_fp_radiis, [idx[1]])
            )
            / 2.0
        ...
        if len(stem_fps) > 0:
            ...
            stem_fps = np.empty(
                shape=[0, self.stem_fp_len * len(self.stem_fp_radiis)], dtype=np.float32
            )
        if len(jbond_fps) > 0:
...
def mpnn_feat(
    mol, ifcoord=True, panda_fmt=False, one_hot_atom=False, donor_features=True
):
