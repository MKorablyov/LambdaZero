...
class EGNNetRLBO(nn.Module):
    def __init__(self, levels=12, num_feat=14, pos_dim=3, edge_attr_dim=0, dim=128, m_dim=128,
                 infer_edges=False, settoset=False, control_exp=False,
                 num_out_per_stem=105, num_out_per_mol=2):
        super().__init__()
        ...
        self.node2stem = nn.Sequential(nn.Linear(feats_dim, feats_dim), _act(), nn.Linear(feats_dim, num_out_per_stem))
        self.node2jbond = nn.Sequential(nn.Linear(feats_dim, feats_dim), _act(), nn.Linear(feats_dim, 1))
