...
class MLPTestActorCritic_thv1(TorchModelV2, nn.Module, ABC):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, **kw):
        ...
        h_size = kw.get("h_size", 1024)
        in_size = obs_space.original_space['mol_graph'].shape[0]
        self.model = nn.Sequential(
            nn.Linear(in_size, h_size*2), nn.ReLU(inplace=True),
            nn.Linear(h_size*2, h_size), nn.ReLU(inplace=True),
            nn.Linear(h_size, h_size), nn.ReLU(inplace=True),
            nn.Linear(h_size, h_size), nn.ReLU(inplace=True),
        )
        self._policy = nn.Sequential(
            nn.Linear(h_size, h_size), nn.ReLU(inplace=True),
            nn.Linear(h_size, action_space.n),
        )
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        if torch.equal(mol_graph, torch.zeros_like(mol_graph)):
            ...
            self._value_out = self._critic(feat).squeeze(1)

            if self.rnd:
                self._value_int = actor_logits[:, 2]
