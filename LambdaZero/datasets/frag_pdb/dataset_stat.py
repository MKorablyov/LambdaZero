...
def make_conn_table(cfg):
    ...
    train_sampler = af.inputs.Sampler(
        train_idxs, probs=ufrag_probs[train_idxs], num_samples=100000
    )
    train_q = torch.utils.data.DataLoader(
        dataset, batch_size=1, num_workers=0, sampler=train_sampler
    )
...
def rdkit_validity():
    ...
    mean_natm = np.asarray(
        [len(fragdb.frag_elem[fragdb.frag_names[name]]) for name in sel_frag_names],
        dtype=np.float,
    ).mean()
