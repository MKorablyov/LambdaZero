...
def train_epoch(loader, model, optimizer, device, max_epoch=np.inf, batch_gradient=1):
    ...
    for bidx, data in enumerate(loader):
        ...
        epoch_y = data.y.detach().cpu().numpy()
...
def val_epoch(loader, model, device):
    ...
    with torch.no_grad():
        for bidx, data in enumerate(loader):
            data = data.to(device)
            y_hat = model(data)
            target_y.append(data.y.detach().cpu().numpy())
            target_dock.append(data.docks.detach().cpu().numpy())
...
class ListGraphDBDataset(Dataset):
    def __init__(self, db, cfg: Namespace):
        ...
        self.bins = False
        if bins is not None and "q100" in db.columns:
...
def run(cfg: Namespace):
    ...
    udf = load_predocked_data(cfg.dataset)
    print("LOADED DB", len(udf))

    train_split_f, test_split_f = cfg.dataset.train_split

    rnd_state = np.random.RandomState(cfg.dataset.seed_split)

    val_graphs = udf.groupby('q100').apply(lambda x: x.sample(frac=test_split_f, random_state=rnd_state)).reset_index(drop=True)
    val_smiles = val_graphs.smiles.unique()
    train_graphs = udf[~udf.smiles.isin(val_smiles)]

    # # ==============================================
    # # Save splits
    # np.save("data/train_val",
    #         dict({"train": train_graphs[["smiles", "dockscore"]], "val": val_graphs[["smiles", "dockscore"]]})
    #         )
    # dataset = np.load("data/train_val.npy", allow_pickle=True).item()
    # train_set = dataset["train"]
    # val_set = dataset["val"]
    # print("Train", train_set.iloc[314]["smiles"], train_set.iloc[314]["dockscore"])
    # print("Validation", val_set.iloc[314]["smiles"], val_set.iloc[314]["dockscore"])
    # # ==============================================
    ...
    if classification:
        print(f"[Train] Class count 0: {(train_graphs['label'] == 0).sum()} |  1: {(train_graphs['label'] == 1).sum()} ")
        print(f"[Test] Class count 0: {(val_graphs['label'] == 0).sum()} |  1: {(val_graphs['label'] == 1).sum()} ")
        train_e, val_e = train_class_epoch, val_class_epoch

    if cfg.dataset.weighted_sampler:
        df = train_graphs

        if classification:
            df = df.join(df.groupby('label').dockscore.count(), on='label', rsuffix='_r')
            df["sample_prob"] = 1. / df["dockscore_r"]
        else:
            cut_bins_cnt = cfg.dataset.weighted_sampler_cbins
            cut_bins, retbins = pd.cut(df.dockscore, bins=cut_bins_cnt, labels=list(range(cut_bins_cnt)), retbins=True)
            df["cbin"] = cut_bins.values.to_numpy()
            # df["cbin_cnt"] = df.groupby("cbin").dockscore.count()
            df = df.join(df.groupby('cbin').dockscore.count(), on='cbin', rsuffix='_r')
            df["sample_prob"] = 1. / df["dockscore_r"]

        weights = torch.DoubleTensor(df["sample_prob"].values)
        batch_sampler = torch.utils.data.sampler.WeightedRandomSampler(weights, batch_size, replacement=False)
        sampler = BSampler(batch_sampler, len(weights))
        # TODO Fix way to use batch_sampler in dataloader with replacement=False

    # do train epochs
    train_set = ListGraphDBDataset(train_graphs, cfg.dataset)
    if batch_sampler is None:
        train_loader = DataLoader(train_set, batch_size=batch_size,
                                  collate_fn=fast_from_data_list, shuffle=True,
                                  num_workers=num_workers)
    else:
        train_loader = DataLoader(train_set,
                                  collate_fn=fast_from_data_list, batch_sampler=sampler,
                                  num_workers=num_workers)

    val_set = ListGraphDBDataset(val_graphs, cfg.dataset)
    val_loader = DataLoader(val_set, batch_size=batch_size, collate_fn=fast_from_data_list,
                            shuffle=False, num_workers=num_workers)

    # Allocate norm
    model._norm = train_set._norm
    ...
    for epoch in range(num_train_epochs):
        ...
        train_seen += train_metrics["train_seen"]
        train_metrics["train_seen"] = train_seen
        summary_stats.update(train_metrics)
        ...
        if lr_sheduler is not None:
            ...
            if do_plot:
                wandb.log({"train_seen": train_seen,
                           "lr_sheduler_cooldown": lr_sheduler.cooldown_counter})
        ...
        if eval_freq > 0 and (epoch+1) % eval_freq == 0:
            ...
            val_metrics["train_seen"] = train_seen
            summary_stats.update(val_metrics)
