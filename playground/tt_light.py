...
def run(args):
    ...
    all_smiles = list(evaluation._eval_data.keys())
    for ksmi in list(evaluation._eval_data.keys()):
        all_smiles += evaluation._eval_data[ksmi]["next"].next_smiles.values.tolist()
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            for iobs, info in enumerate(infos):
                if done[iobs]:
                    froms, tos = all_smiles.index(info['act_molecule']), all_smiles.index(info['res_molecule'])
                    env_pairs.append((froms, tos))
                    env_r.append(reward[iobs])
            ...
            # print(f"Unique p: {len(set(env_pairs))} | uniq r: {len(set(env_r))}")
